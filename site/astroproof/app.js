...
let state={profile:null,records:[]},chart=null,report=null,timer,nowRange='week';
...
function save(){localStorage.setItem(KEY,JSON.stringify(state))}function load(){try{state=JSON.parse(localStorage.getItem(KEY)||'{"profile":null,"records":[]}')}catch{state={profile:null,records:[]}}}
function page(id){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id===id));document.querySelectorAll('#nav button').forEach(x=>x.classList.toggle('active',x.dataset.page===id));$('crumb').textContent='PERSONAL ATLAS / '+id.toUpperCase();window.scrollTo(0,0);if(id==='reports')renderReport();if(id==='timeline')renderTimeline();if(id==='council')renderCouncil();if(id==='ledger')renderLedger()}
...
function planetary(date){const names=['Sun','Moon','Mercury','Venus','Mars','Jupiter','Saturn','Uranus','Neptune','Pluto'];const out={};for(const name of names){const body=Astronomy.Body[name];const vec=Astronomy.GeoVector(body,date,true);const ecl=Astronomy.Ecliptic(vec);out[name]=signInfo(ecl.elon)}return out}
...
function renderOverview(){
  ...
  const item=h=>`<div class="report-section"><h3>${TOPICS[h.topic]?.[1]||'Life'} · ${h.mover} ${h.aspect} natal ${h.target}</h3><p class="muted">Peak sample: ${h.date} · orb ${h.orb.toFixed(2)}° · priority ${h.score.toFixed(1)}</p><p>${esc(forecastMeaning(h))}</p><p><b>Why it ranked:</b> ${h.dasha_support.count?h.dasha_support.count+' Vimshottari layer(s) also activate this life area: '+h.dasha_support.levels.map(esc).join(', '):'transit strength/exactness only; no direct MD/AD/PD house activation found.'}</p><span class="hint">Traditional timing interpretation; priority rank is not an event probability.</span></div>`;
  dash.innerHTML=`
